...
const getServideskInc = async (issInc) => {
  ...
  if (issInc.length > 0) {
    issInc.forEach((item) => {
      ...
      item.FechaApertura = item.FechaApertura
        ? convertTimestampToDate(item.FechaApertura)
        : null;
      item.FechaCierre = item.FechaCierre
        ? convertTimestampToDate(item.FechaCierre)
        : null;
  ...
  static async getIssIncidents() {
    try {
      ...
      throw new DatabaseError(
        "Failed to fetch servidesk from the database: getIssIncidents()",
        error
      );
    }
  }
